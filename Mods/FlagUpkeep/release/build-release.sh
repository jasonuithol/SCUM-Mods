#!/usr/bin/env bash
# Build a FlagUpkeep EVAL package: the engine (Config + the shared gating lib +
# upkeep) is encrypted and BAKED INTO upkeep-core.exe, decrypted in memory at
...
# No payload.bin, and no clean Config.lua/upkeep.lua/gating.lua, are shipped.
#
...
mod="$(cd "$here/.." && pwd)"              # Mods/FlagUpkeep
scripts="$mod/Scripts"
gating="$mod/../shared/Scripts/gating.lua" # shared lib (baked into the engine)
out="$mod/dist/FlagUpkeep"                 # under dist/ -> gitignored
...
"$CXX" -O2 -s -static -static-libgcc -static-libstdc++ -fno-exceptions -fno-rtti \
    -o "$build/packer.exe" "$here/upkeep-core.cpp" -lkernel32

# 2. pack the clean engine (Config + gating + upkeep) into an embeddable C header
...
moddir='C:\scumserver\SCUM\Binaries\Win64\ue4ss\Mods\FlagUpkeep'
"$build/packer.exe" wrap "$(cygpath -w "$here/bootstrap.lua")" "$(cygpath -w "$out/Scripts/main.lua")" "$moddir"

# 4. compile the FINAL upkeep-core.exe with the engine BAKED IN + the time-bomb
#    (so the shipped package needs no payload.bin)
"$CXX" -O2 -s -static -static-libgcc -static-libstdc++ -fno-exceptions -fno-rtti \
    -DFU_EMBEDDED -I"$(cygpath -m "$build")" -DFU_EXPIRY="$expiry" \
    -o "$out/upkeep-core.exe" "$here/upkeep-core.cpp" -lkernel32
...
zipout="$mod/dist/FlagUpkeep-eval.zip"
rm -f "$zipout"
...
echo "  package: Scripts/main.lua (obfuscated) + upkeep-core.exe (engine embedded)"
