...
<#
.SYNOPSIS
  Downloads GarbageGoober's runtime dependencies into this folder.

.DESCRIPTION
  The mod needs to read SCUM.db (SQLite), but these binaries are deliberately
  NOT committed to git. Run this once after cloning or deploying the mod; it
  fetches each dependency from its official source and verifies it against a
...
# which file to pull out of the archive, and where to put it. Add more here as
# the mod grows.
$Dependencies = @(
    [pscustomobject]@{
        Name      = 'sqlite3'
...
function Install-Dependency {
    ...
    $tmpDir = Join-Path $env:TEMP ('gg_dl_' + [guid]::NewGuid().ToString('N'))
...
Write-Host 'GarbageGoober - installing runtime libraries into:'
Write-Host ("  {0}" -f $Here)
